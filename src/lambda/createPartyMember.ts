...
import {httpError, HttpError, Conflict, NotPermitted} from '../util/errors';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    // Ensure party is not in progress
    if (party.status !== 'waiting') throw new NotPermitted('Cannot join a party currently in progress');
